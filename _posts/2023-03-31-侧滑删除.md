...
title:      侧滑删除
subtitle:   侧滑删除
date:       2023-03-31 10:06:01 GMT+0800
author:     920
header-img: 
catalog: true
stickie: false
...
```
<view class="list-item skeleton-rect" v-for="(item,index) in listData" :key="index">
    <rk-movable :options="options" :index="index" @content-click="goDetail(item)" @click="clickActionsBtn">
        <house-item :houseItem="item"></house-item>
    </rk-movable>
</view>
```
**options**
 ...
 * 侧滑按钮点击事件
 */
clickActionsBtn(row, idx) {
    
    // console.log('点击了', row, '行', '点击了idx', idx, '个');
    let item = this.listData[row];
    
},
```

#### 组件
>参考`uviewui`中的`swipeAction`有部分改动
    ...
    export default {
        ...
        data() {
            return {
                ...
                elId: 'rk_movable_list_el', // id，用于通知另外组件关闭时的识别
        ...
        computed: {
            ...
            btnStyle() {
                return style => {
                    let css = {};
                    style.width = Number(this.btnWidth)-1 + 'rpx';
                    return style;
                };
            }
        },
        ...
        methods: {
            ...
            getActionRect() {
                ...
                query.select('.u-swipe-action').boundingClientRect(res => {
